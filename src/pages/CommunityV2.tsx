...
type Group=CreatedGroup&{role?:string;createdBy?:string;muted?:boolean;archived?:boolean;favorite?:boolean;memberCount?:number;description?:string};
...
export default function CommunityV2(){
 ...
 const loadGroups=useCallback(async(uid:string)=>{
  ...
  const{data:m,error:me1}=await db.from("community_group_members").select("group_id,role,muted,archived,favorite").eq("user_id",uid);
