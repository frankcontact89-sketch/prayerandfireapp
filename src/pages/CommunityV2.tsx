...
export default function CommunityV2(){
 ...
 const chatBackgroundLabel=L("Chat background","Fondo del chat","Plano de fundo do chat");
 const defaultBackgroundLabel=L("Default","Predeterminado","Padrão");
 const warmBackgroundLabel=L("Warm dark","Oscuro cálido","Escuro quente");
 const fireBackgroundLabel=L("Prayer & Fire","Prayer & Fire","Prayer & Fire");
 const photoBackgroundLabel=L("Choose photo","Elegir foto","Escolher foto");
 const resetBackgroundLabel=L("Reset background","Restablecer fondo","Redefinir fundo");
 ...
 const file=useRef<HTMLInputElement>(null),photo=useRef<HTMLInputElement>(null),wallpaperFile=useRef<HTMLInputElement>(null),end=useRef<HTMLDivElement>(null);
 ...
 const wallpaperKey=(groupId:string)=>`pf_chat_bg_${groupId}`;
 useEffect(()=>{
  if(!selected){setChatBg("default");return}
  try{setChatBg(localStorage.getItem(wallpaperKey(selected.id))||"default")}catch{setChatBg("default")}
 },[selected?.id]);
 const saveChatBg=(value:string)=>{
  if(!selected)return;
  setChatBg(value);
  try{localStorage.setItem(wallpaperKey(selected.id),value)}catch{/* ignore */}
 };
 const chooseWallpaper=(f?:File)=>{
  if(!f||!selected)return;
  if(f.size>3*1024*1024){toast(L("Choose an image under 3 MB","Elige una imagen de menos de 3 MB","Escolha uma imagem com menos de 3 MB"));return}
 ...
 if(selected)return <div className="fixed inset-0 flex flex-col bg-background text-foreground" style={{paddingTop:"env(safe-area-inset-top)"}}>
  ...
  <main className="flex-1 min-h-0 overflow-y-auto px-3 py-3 space-y-2.5" style={chatBackgroundStyle}>
    ...
    <div ref={el=>{msgRefs.current[m.id]=el}} className={`relative flex ${m.mine?"justify-end":"justify-start"} ${reactBar?.id===m.id?"z-40":""} ${highlightMsg===m.id?"rounded-2xl ring-2 ring-orange-400/70":""}`}>
     ...
     <div
      ...
      className={`group relative max-w-[86%] select-none px-3 py-2.5 shadow-sm ${(reactions[m.id]||[]).length?"mb-6":""} ${m.mine?"rounded-2xl rounded-tr-md bg-primary text-primary-foreground":"rounded-2xl rounded-tl-md border border-border bg-card text-card-foreground"}`}
     >
  ...
  <div className="shrink-0 border-t border-border bg-background px-2 pt-2 pb-[max(8px,env(safe-area-inset-bottom))]">
    ...
    {messageInfo&&<div className="fixed inset-0 z-[70] overflow-y-auto bg-background text-foreground" style={{paddingTop:"env(safe-area-inset-top)",paddingBottom:"env(safe-area-inset-bottom)"}}>
     ...
     <div className="mx-auto max-w-xl px-4 pb-8 pt-6">
      <div className="flex justify-end pb-6">
       <div className="max-w-[88%] rounded-2xl rounded-tr-md bg-primary px-3 py-2.5 text-primary-foreground shadow-sm">
