...
export default function CommunityV2(){
 ...
 const memberUpdate=async(ch:any)=>{if(!selected||!me)return;const{muted,archived,favorite}=ch;const safe:any={};if(muted!==undefined)safe.muted=muted;if(archived!==undefined)safe.archived=archived;if(favorite!==undefined)safe.favorite=favorite;if(!Object.keys(safe).length)return;const{error}=await db.from("community_group_members").update(safe).eq("group_id",selected.id).eq("user_id",me.id);if(error){toast(error.message||actionFailedLabel);return}setSelected(s=>s?{...s,...ch}:s);setGroups(v=>v.map(g=>g.id===selected.id?{...g,...ch}:g));toast(savedLabel)};
