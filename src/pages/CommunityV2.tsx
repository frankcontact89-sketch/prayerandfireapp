...
import{Archive,ArrowLeft,Ban,Bell,BellOff,Camera,CheckCheck,Copy,FileText,Flag,ChevronRight,CornerUpLeft,Link2,LogOut,Mic,MoreHorizontal,Paperclip,Plus,Search,Send,Settings,ShieldCheck,Star,Trash2,UserPlus,Users,X}from"lucide-react";
...
type Group=CreatedGroup&{role?:string;createdBy?:string;muted?:boolean;archived?:boolean;memberCount?:number;description?:string};
...
export default function CommunityV2(){
 ...
 const[confirmLeave,setConfirmLeave]=useState(false),[showArchived,setShowArchived]=useState(false),[busyGroup,setBusyGroup]=useState(false);
 ...
 const memberUpdate=async(ch:any)=>{if(!selected||!me)return;const{muted,archived,favorite}=ch;const safe:any={};if(muted!==undefined)safe.muted=muted;if(archived!==undefined)safe.archived=archived;if(favorite!==undefined)safe.favorite=favorite;if(!Object.keys(safe).length)return;const{error}=await db.from("community_group_members").update(safe).eq("group_id",selected.id).eq("user_id",me.id);if(error){toast(error.message||t.actionFailed);return}setSelected(s=>s?{...s,...ch}:s);setGroups(v=>v.map(g=>g.id===selected.id?{...g,...ch}:g));toast(archived===true?t.archivedDone:archived===false?t.unarchivedDone:t.savedDone)};
 const saveGroup=async()=>{if(!selected||!canManageGroup(selected))return;const{error}=await db.from("community_groups").update({name:name.trim()||selected.name,description:desc.trim(),updated_at:new Date().toISOString()}).eq("id",selected.id);if(error){toast(error.message||t.actionFailed);return}setSelected(s=>s?{...s,name:name.trim()||s.name,description:desc.trim(),subtitle:desc.trim()||s.subtitle}:s);setGroups(v=>v.map(g=>g.id===selected.id?{...g,name:name.trim()||g.name,description:desc.trim()}:g));setEdit(false);toast(t.savedDone)};
 const changePhoto=async(f?:File)=>{if(!f||!selected||!me||!canManageGroup(selected))return;const path=`${me.id}/groups/${selected.id}-${Date.now()}.jpg`;const up=await supabase.storage.from("community-media").upload(path,f,{contentType:f.type,upsert:true});if(up.error){toast(up.error.message||t.actionFailed);return}const{error}=await db.from("community_groups").update({avatar_url:path}).eq("id",selected.id);if(error){toast(error.message||t.actionFailed);return}const url=await signed(path);setSelected(s=>s?{...s,avatar:url}:s);setGroups(v=>v.map(g=>g.id===selected.id?{...g,avatar:url}:g));toast(t.photoUpdated)};
 const deleteGroupNow=async()=>{if(!selected||!me||!canDeleteGroup(selected)){toast(t.noPermission);return}setBusyGroup(true);const{error}=await db.from("community_groups").delete().eq("id",selected.id);setBusyGroup(false);if(error){toast(error.message||t.actionFailed);return}setConfirmDelGroup(false);setSelected(null);setInfo(false);setEdit(false);loadGroups(me.id);toast(t.groupDeleted)};
 const isGroupOwner=(g:Group|null)=>!!g&&!!me&&(g.createdBy===me.id||g.role==="owner");
 const leave=async()=>{if(!selected||!me)return;if(isGroupOwner(selected)){setConfirmLeave(false);toast(t.ownerCannotLeave);return}setBusyGroup(true);const{error}=await db.from("community_group_members").delete().eq("group_id",selected.id).eq("user_id",me.id);setBusyGroup(false);setConfirmLeave(false);if(error){toast(error.message||t.actionFailed);return}setSelected(null);setInfo(false);loadGroups(me.id);toast(t.leftGroup)};
 ...
 return <div className="fixed inset-0 bg-black text-white overflow-hidden" style={{paddingTop:"env(safe-area-inset-top)",paddingBottom:"env(safe-area-inset-bottom)"}}><div className="max-w-xl mx-auto h-full bg-[#080808] flex flex-col"><header className="shrink-0 px-4 pt-3 pb-3 border-b border-white/5"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3 min-w-0"><button onClick={goBack} aria-label={t.back} className="w-10 h-10 rounded-full bg-zinc-900 grid place-items-center shrink-0"><ArrowLeft className="w-5 h-5"/></button><img src={entryLogo} alt="Prayer & Fire" className="w-11 h-11 rounded-full object-cover shrink-0 bg-zinc-900"/><div className="min-w-0"><div className="text-[10px] tracking-[.2em] text-orange-400 font-bold">PRAYER &amp; FIRE</div><h1 className="text-xl font-black truncate">{t.title}</h1>{me&&<div className="text-xs text-zinc-500 truncate">{me.name}</div>}</div></div><div className="flex items-center gap-2 shrink-0">{isStaff&&<button onClick={()=>setPanel(true)} aria-label={t.requestsPanel} className="relative w-11 h-11 rounded-full bg-zinc-900 text-orange-400 grid place-items-center"><ShieldCheck className="w-5 h-5"/>{pendingCount>0&&<span className="absolute -top-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-orange-500 text-black text-[11px] font-black grid place-items-center">{pendingCount}</span>}</button>}<button onClick={()=>canCreate?setCreate(true):toast(t.onlyAdminsCreate)} className={`w-11 h-11 rounded-full grid place-items-center ${canCreate?"bg-orange-500 text-black":"bg-zinc-900 text-zinc-500"}`} aria-label={t.new}><Plus className="w-5 h-5"/></button></div></div></header><div className="shrink-0 px-4 py-3"><div className="bg-zinc-900 border border-white/10 rounded-2xl h-12 px-4 flex items-center gap-2"><Search className="w-5 h-5 text-zinc-500"/><input value={q} onChange={e=>setQ(e.target.value)} placeholder={t.search} className="bg-transparent outline-none flex-1 min-w-0"/></div></div>{(archivedCount>0||showArchived)&&<button onClick={()=>setShowArchived(v=>!v)} className="mt-3 w-full h-11 rounded-2xl bg-zinc-900 border border-white/10 px-4 flex items-center gap-2 text-sm"><Archive className="w-4 h-4 text-orange-400"/><span className="flex-1 text-left">{showArchived?t.backToChats:t.archivedChats}</span>{!showArchived&&<span className="text-zinc-500">{archivedCount}</span>}</button>}<div className="flex-1 min-h-0 overflow-y-auto px-3 pb-8">{listLoading?<div className="py-20 text-center text-zinc-500 text-sm">{t.loading}</div>:listError?<div className="py-20 text-center px-8"><p className="text-sm text-zinc-400">{t.loadError}</p><button onClick={()=>me&&loadGroups(me.id)} className="mt-4 h-11 px-6 rounded-2xl bg-orange-500 text-black font-black">{t.retry}</button></div>:visible.length===0?<div className="py-20 text-center px-8"><div className="w-16 h-16 rounded-full bg-orange-500/10 text-orange-500 grid place-items-center mx-auto mb-4"><Users className="w-7 h-7"/></div><h2 className="font-bold text-lg">{showArchived?t.noArchived:t.empty}</h2><p className="text-sm text-zinc-500 mt-2">{canCreate?t.sub:`${t.sub} ${t.onlyAdminsCreate}`}</p></div>:visible.map(g=><button key={g.id} onClick={()=>setSelected(g)} className="w-full text-left flex gap-3 px-2 py-3.5 border-b border-white/10 active:bg-white/5 rounded-xl"><img src={g.avatar||entryLogo} alt="" className="w-14 h-14 rounded-full object-cover"/><div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><div className="font-extrabold truncate">{g.name}</div><span className={`text-[11px] ${g.unread>0?"text-orange-400 font-bold":"text-zinc-500"}`}>{g.lastTime}</span></div><div className="flex justify-between items-center gap-2"><div className={`text-sm truncate mt-1 ${g.unread>0?"text-zinc-100 font-semibold":"text-zinc-400"}`}>{g.subtitle}</div>{g.unread>0&&<span className="min-w-5 h-5 px-1.5 rounded-full bg-orange-500 text-black text-[11px] font-black grid place-items-center shrink-0">{g.unread}</span>}</div></div></button>)}</div>{noAccessGroup&&<div className="fixed inset-0 z-[130] bg-black/80 grid place-items-center px-8" onClick={()=>setNoAccessGroup(null)}><div onClick={e=>e.stopPropagation()} className="w-full max-w-sm rounded-3xl bg-zinc-950 border border-white/10 p-6 text-center"><img src={noAccessGroup.avatar||entryLogo} alt="" className="w-16 h-16 rounded-full object-cover mx-auto"/><b className="block mt-4 text-lg">{noAccessGroup.name}</b><p className="mt-1 text-xs text-zinc-500">{noAccessGroup.memberCount} {t.members}</p><b className="block mt-4 text-orange-400 text-sm">{t.noAccessTitle}</b><p className="mt-2 text-sm text-zinc-400">{t.noAccessBody}</p><button onClick={()=>setNoAccessGroup(null)} className="mt-6 w-full h-12 rounded-2xl bg-zinc-900">{t.back}</button></div></div>}{canCreate&&<CreateGroupModal open={create} onClose={()=>setCreate(false)} onCreate={createGroup} language={lang}/>}</div></div>;
}
