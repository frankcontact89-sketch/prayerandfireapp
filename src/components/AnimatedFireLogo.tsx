...
import fireLogo from "@/assets/prayer-fire-background.png";

export function AnimatedFireLogo() {
  return (
    <div className="relative inline-block">
      {/* Background layer with very subtle glow */}
      <div className="absolute inset-0 blur-3xl bg-primary/10 animate-pulse"></div>
      
      {/* Fire logo as background - more transparent and subtle */}
      <img 
        src={fireLogo} 
        alt="Prayer & Fire" 
        className="relative w-32 h-32 opacity-40 drop-shadow-[0_0_15px_rgba(255,106,0,0.2)]"
      />
      
      {/* Animated flame particles in front - these create the living fire effect */}
      ...
      <div className="absolute top-4 left-1/2 w-1 h-1 bg-accent rounded-full opacity-80 animate-[flameRise_2.1s_ease-in-out_infinite_0.4s]"></div>
      <div className="absolute top-7 left-[48%] w-1 h-1 bg-primary rounded-full opacity-70 animate-[flameRise_1.9s_ease-in-out_infinite_0.7s]"></div>
    </div>
  );
}
