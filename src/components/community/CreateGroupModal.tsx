...
export default function CreateGroupModal({ open, onClose, onCreate, language }: Props) {
  ...
  return <div className="fixed inset-0 z-[100] bg-[#080808] text-white" style={{paddingTop:"env(safe-area-inset-top)",paddingBottom:"env(safe-area-inset-bottom)"}}>
    <header className="h-16 px-4 flex items-center gap-3 border-b border-white/10 bg-black/95 backdrop-blur-xl">
      ...
      <div className="flex-1"><h2 className="font-black text-lg">{t.newGroup}</h2><p className="text-xs text-zinc-400">{step===1?t.choose:t.subject}</p></div>
      <button onClick={close} className="w-9 h-9 grid place-items-center text-zinc-400"><X/></button>
    </header>

    {step===1 ? <main className="px-5 py-4 max-w-xl mx-auto h-[calc(100%-4rem)] overflow-y-auto pb-24">
      <div className="h-12 px-4 rounded-2xl bg-zinc-900 border border-white/10 flex items-center gap-2"><Search className="text-zinc-500"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={t.search} className="flex-1 bg-transparent outline-none min-w-0"/></div>
      {chosenPeople.length>0&&<div className="flex gap-2 overflow-x-auto py-4">{chosenPeople.map(p=><button key={p.id} onClick={()=>setChosen(v=>v.filter(x=>x!==p.id))} className="shrink-0 px-3 py-2 rounded-full bg-orange-500/15 border border-orange-500/30 text-orange-300 text-xs">{p.name} ×</button>)}</div>}
      <div className="text-xs uppercase tracking-widest text-zinc-500 font-bold py-3">{chosen.length} {t.selected}</div>
      {loading ? <div className="text-zinc-500 py-10 text-center">{t.loading}</div> : filtered.length===0 ? <div className="text-zinc-500 py-10 text-center">{t.noPeople}</div> : filtered.map(p=>{const active=chosen.includes(p.id);return <button key={p.id} onClick={()=>setChosen(v=>active?v.filter(x=>x!==p.id):[...v,p.id])} className="w-full flex items-center gap-3 py-3 border-b border-white/10">
        <div className="w-12 h-12 rounded-full overflow-hidden bg-gradient-to-br from-orange-500/30 to-zinc-800 grid place-items-center font-black">{p.avatar?<img src={p.avatar} className="w-full h-full object-cover" alt=""/>:p.name[0]?.toUpperCase()}</div>
        <div className="flex-1 text-left min-w-0"><div className="font-semibold truncate">{p.name}</div>{p.email&&<div className="text-xs text-zinc-500 truncate">{p.email}</div>}</div>
        <div className={`w-6 h-6 rounded-full border grid place-items-center ${active?"bg-orange-500 border-orange-500 text-black":"border-zinc-600"}`}>{active&&<Check className="w-4 h-4"/>}</div>
      </button>})}
      <button onClick={()=>setStep(2)} className="fixed bottom-[max(22px,env(safe-area-inset-bottom))] right-5 px-6 h-12 rounded-full bg-orange-500 text-black font-black">{t.next}</button>
    </main> : <main className="px-5 py-8 max-w-xl mx-auto overflow-y-auto pb-24">
