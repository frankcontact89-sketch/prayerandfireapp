...
export default function CommunityAdminPanel({ t, meId, isOwner, onClose, onChanged }: Props) {
  ...
  const removeMember = async (userId: string) => { if (!manageGroup || userId === manageGroup.created_by) return; if (!window.confirm(t.removeMember + "?")) return; setBusy(userId); await db.from("community_group_members").delete().eq("group_id", manageGroup.id).eq("user_id", userId); setBusy(null); await loadGroupMembers(manageGroup.id); await load(); onChanged(); };
  ...
  const muteForMe = async () => { if (!manageGroup) return; const mine = groupMembers.find(m => m.user_id === meId); setBusy(manageGroup.id); await db.rpc("set_community_membership_preferences", { _group_id: manageGroup.id, _muted: !mine?.muted }); setBusy(null); await loadGroupMembers(manageGroup.id); onChanged(); };
  ...
  const title = tab === "requests" ? t.requestsPanel : tab === "admins" ? t.manageAdmins : tab === "groups" ? t.groups : t.moderation;
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-[#080808] text-white flex flex-col" style={{ paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}>
      ...
      {manageGroup && <div className="fixed inset-0 z-[125] bg-[#080808] text-white flex flex-col" style={{ paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}>
        <header className="h-16 px-3 flex items-center gap-3 border-b border-white/10"><button onClick={() => setManageGroup(null)} className="w-10 h-10 rounded-full bg-zinc-900 grid place-items-center"><ArrowLeft /></button><div className="flex-1"><b className="block truncate">{manageGroup.name}</b><span className="text-xs text-zinc-500">{t.groupAdministration}</span></div></header>
        <div className="flex-1 overflow-y-auto px-4 pb-10">
          <div className="grid grid-cols-2 gap-2 py-4">
            <button onClick={() => { setEditGroup(manageGroup); setEditName(manageGroup.name); setEditDescription(manageGroup.description || ""); }} className="h-12 rounded-xl bg-zinc-900 flex items-center justify-center gap-2"><Pencil className="w-4 h-4" /> {t.edit}</button>
            <button onClick={muteForMe} className="h-12 rounded-xl bg-zinc-900 flex items-center justify-center gap-2"><BellOff className="w-4 h-4" /> {t.muteForMe}</button>
            <button onClick={() => toggleGroupHidden(true)} className="h-12 rounded-xl bg-zinc-900 flex items-center justify-center gap-2"><EyeOff className="w-4 h-4" /> {t.hideGroup}</button>
            <button onClick={() => toggleGroupHidden(false)} className="h-12 rounded-xl bg-zinc-900 flex items-center justify-center gap-2"><Eye className="w-4 h-4" /> {t.showGroup}</button>
            <button onClick={() => setDeleteGroup(manageGroup)} className="col-span-2 h-12 rounded-xl bg-red-500/15 text-red-300 flex items-center justify-center gap-2"><Trash2 className="w-4 h-4" /> {t.deleteGroup}</button>
          </div>

          <div className="text-xs uppercase tracking-widest text-zinc-500 font-bold py-3">{t.currentMembers}</div>
          {groupMembers.map(m => <div key={m.user_id} className="flex items-center gap-3 py-3 border-b border-white/10"><Avatar src={m.avatar} name={m.name} /><div className="flex-1 min-w-0"><div className="font-semibold truncate">{m.name}</div><div className="text-xs text-zinc-500 truncate">{m.email || ""} · {m.role}</div></div>{m.user_id !== manageGroup.created_by && <><button onClick={() => setGroupRole(m.user_id, m.role === "admin" ? "member" : "admin")} className="w-10 h-10 rounded-full bg-zinc-900 text-orange-400 grid place-items-center" aria-label={t.toggleGroupAdmin}><ShieldCheck className="w-4 h-4" /></button><button onClick={() => removeMember(m.user_id)} className="w-10 h-10 rounded-full bg-zinc-900 text-red-400 grid place-items-center" aria-label={t.removeMember}><UserMinus className="w-4 h-4" /></button></>}</div>)}

          <div className="text-xs uppercase tracking-widest text-zinc-500 font-bold pt-6 pb-3">{t.addApproved}</div>
          <input value={memberSearch} onChange={e => setMemberSearch(e.target.value)} placeholder={t.searchPeople} className="w-full h-12 rounded-xl bg-zinc-900 px-4 outline-none" />
      ...
      {deleteGroup && <div className="fixed inset-0 z-[140] bg-black/80 grid place-items-center px-8" onClick={() => setDeleteGroup(null)}><div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-3xl bg-zinc-950 border border-white/10 p-6"><b className="text-lg">{t.deleteGroup}: {deleteGroup.name}</b><p className="mt-2 text-sm text-zinc-400">{t.deleteGroupBody}</p><div className="mt-6 flex gap-3"><button onClick={() => setDeleteGroup(null)} className="flex-1 h-12 rounded-2xl bg-zinc-900">{t.cancel}</button><button disabled={busy === deleteGroup.id} onClick={confirmDeleteGroup} className="flex-1 h-12 rounded-2xl bg-red-500 text-black font-black disabled:opacity-50">{t.delete}</button></div></div></div>}
    </div>
  );
}
