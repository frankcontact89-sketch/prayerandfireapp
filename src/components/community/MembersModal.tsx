...
import { ArrowLeft, Check, Crown, ShieldCheck, Trash2, User } from "lucide-react";
...
export default function MembersModal({ t, groupId, mode, canManage, onClose, onChanged }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const { data: profs } = eligible.length ? await db.from("profiles").select("id,username,email,avatar_url").in("id", eligible) : { data: [] };
  ...
  const addMembers = async () => {
    ...
    setBusy(true);
    await db.from("community_group_members").insert(chosen.map((id) => ({ group_id: groupId, user_id: id, role: "member" })));
    setBusy(false);
    onChanged();
    onClose();
  };
