    ...
    return (
      <div className="max-w-2xl mx-auto p-6 space-y-4">
        <div className="flex items-center gap-4 mb-6">
          <button onClick={() => setShowWhatsAppContacts(false)} className="text-primary hover:text-primary/80 transition-colors"><ArrowLeft className="w-6 h-6" /></button>
          <h2 className="text-2xl font-bold text-foreground">💬 {t("whatsappContacts")}</h2>
        </div>
        {whatsappLinks.map((link) => <button key={link.id} onClick={() => window.open(link.url, "_blank")} className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-4 px-6 rounded-xl flex items-center justify-center gap-2 transition-colors"><MessageCircle className="w-5 h-5" />{link.title}</button>)}
        <button onClick={() => setShowWhatsAppContacts(false)} className="mt-6 flex items-center justify-center gap-2 w-full text-primary hover:text-primary/80 font-semibold transition-all hover:scale-110 active:scale-95"><Flame className="w-5 h-5 animate-pulse" /></button>
